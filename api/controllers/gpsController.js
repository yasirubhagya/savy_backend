...
exports.gps_get_byTrackerId = (req, res, next) => {
    ...
    gps.find({ trackerId: req.params.trackerId },{ latitude: 1, longitude: 1, _id: 0 }).sort({ "uDate": -1 }).limit(500).exec()
    .then(result=>{
        res.status(200).json(result);
    })
    .catch(err=>{
...
exports.gps_get_live_byTrackerId = (req, res, next) => {
    ...
    gps.find({ trackerId: req.params.trackerId }, { latitude: 1, longitude: 1, _id: 0 }).sort({ "uDate": -1 }).limit(1).exec()
    .then(result=>{
        res.status(200).json(result);
    })
    .catch(err=>{
